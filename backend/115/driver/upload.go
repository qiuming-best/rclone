...
import (
	...
	hash "github.com/rclone/rclone/backend/115/driver/crypto"
	cipher "github.com/rclone/rclone/backend/115/driver/crypto/ec115"
...
func (c *Pan115Client) checkUploadStatus(dirID, sha1 string) error {
	// 验证上传是否成功
	// 115 OSS 上传后文件列表更新可能存在延迟，此处引入重试机制以避免假性上传失败
	for retry := 0; retry < 15; retry++ {
		if retry > 0 {
			// Backoff strategy: wait 1s, 2s, 3s... etc.
		...
		opts := []GetFileOptions{
			...
			WithAsc(false),
			WithLimit(1000), // Increase limit to find file even if many others were uploaded
		}
